...
-- Habitaciones base para entorno local
INSERT INTO rooms (code, name, subtitle, category, description, capacity, size_m2, bed_type, status, is_featured)
VALUES
...
-- Tarifa vigente global de 2 anios por habitacion
INSERT INTO room_rates (room_id, valid_during, nightly_rate_amount, currency, note)
SELECT r.id, daterange(current_date, current_date + 730, '[)'), v.price, 'MXN', 'Tarifa base dev local'
FROM (
  VALUES
...
WHERE NOT EXISTS (
  ...
  WHERE rr.room_id = r.id
    AND rr.valid_during && daterange(current_date, current_date + 730, '[)')
);
