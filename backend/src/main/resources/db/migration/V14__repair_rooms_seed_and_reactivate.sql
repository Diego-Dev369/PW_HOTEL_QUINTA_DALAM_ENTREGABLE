...
-- V14: reparar seed de habitaciones en caso de soft-delete o data drift en produccion.
-- Si una suite existe por code pero fue soft-deleted, se reactiva y se limpia deleted_at.
...
SET
  ...
  status = 'ACTIVE'::hotel.room_status,
  is_featured = EXCLUDED.is_featured,
  deleted_at = NULL,
  updated_at = NOW();

-- Garantiza al menos una tarifa activa por suite para rango amplio de fechas.
INSERT INTO hotel.room_rates (room_id, valid_during, nightly_rate_amount, currency, note)
SELECT r.id, daterange(DATE '2025-01-01', DATE '2035-01-01', '[)'), v.price, 'MXN', 'Tarifa base seed repair V14'
FROM (
  VALUES
